...
export class CartController implements OnModuleInit {
    ...
    private initializeCircuitBreakers() {
      ...
      this.addToCartCircuit.fallback(() => {
        throw new ServiceUnavailableException('Add to cart service is temporarily unavailable. Please try again later.');
      });
      ...
      this.getCartCircuit.fallback(() => {
        throw new ServiceUnavailableException('Get cart service is temporarily unavailable. Please try again later.');
      });
      ...
      this.updateCartItemCircuit.fallback(() => {
        throw new ServiceUnavailableException('Update cart item service is temporarily unavailable. Please try again later.');
      });

        this.removeCartItemCircuit = this.circuitBreakerService.createCircuitBreaker(
        async (productId: string) => {
          return await lastValueFrom(
            this.cartService.send({ cmd: 'remove_from_cart' }, {productId}).pipe(timeout(5000))
          );
        },
        {
          timeout: 5000,
      ...
      this.removeCartItemCircuit.fallback(() => {
        throw new ServiceUnavailableException('Remove cart item service is temporarily unavailable. Please try again later.');
      });

      this.clearCartCircuit = this.circuitBreakerService.createCircuitBreaker(
        async (userId: string) => {
          return await lastValueFrom(
            this.cartService.send({ cmd: 'clear_cart' }, { userId }).pipe(timeout(5000))
          );
        },
        {
          timeout: 5000,
      ...
      this.clearCartCircuit.fallback(() => {
        throw new ServiceUnavailableException('Clear cart service is temporarily unavailable. Please try again later.');
      });
    ...
    async updateCartItem(@Param('productId') productId: string, @Body() body: UpdateCartQuantityDto, @Req() req) {
