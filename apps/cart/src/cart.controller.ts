...
export class CartController {
  ...
  addToCart(@Payload() data: { userId: string, productId: string, quantity: number, idempotencyKey?: string }) {
  ...
  removeFromCart(@Payload() data: { userId: string, productId: string, idempotencyKey?: string }){
    const idempotencyKey = data.idempotencyKey || this.generateIdempotencyKey(
      { userId: data.userId, productId: data.productId },
      'remove'
    );
    return this.cartService.removeFromCart(data.userId, data.productId, idempotencyKey);
  }

  @MessagePattern({ cmd: 'clear_cart' })
  clearCart(@Payload() data: { userId: string, idempotencyKey?: string }){
    const idempotencyKey = data.idempotencyKey || this.generateIdempotencyKey(
      { userId: data.userId },
      'clear'
    );
    return this.cartService.clearCart(data.userId, idempotencyKey);
  }

  @MessagePattern({ cmd: 'update_cart_item' })
  updateCartItem(@Payload() data: { userId: string, productId: string, quantity: number, idempotencyKey?: string }){
    const idempotencyKey = data.idempotencyKey || this.generateIdempotencyKey(
      { userId: data.userId, productId: data.productId, quantity: data.quantity },
      'update'
    );
    return this.cartService.updateCartItem(data.userId, data.productId, data.quantity, idempotencyKey);
  }
